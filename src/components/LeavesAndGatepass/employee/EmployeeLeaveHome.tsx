...
export const EmployeeLeaveHome = () => {
  ...
  const allRejectedList = useSelector((state: any) => state.leave.rejectedLeaves);
  ...
  console.log("allRejectedList", allRejectedList)
          ...
          <div className="">
                        ...
                        <div className='flex gap-4'>
                          ...
                          <button
                            onClick={() => dispatch(updatePendingGatePassAsync({
                              ...
                              status: "rejected",
                              gatePassDate: (element.gatePassDate),
                              gatePassTime: convertTimeToAMPM(element.time)
                            })).then(() => {
                              dispatch(getAllPendingGatePassAsync());
                              dispatch(getAllAcceptedGatePassAsync())
                              dispatch(getAllRejectedGatePassAsync());
            ...
            {activeButton === 'Rejected' && (
              <div>
                {/* usemap on this */}
                {/* LEAVE */}
                {allRejectedList && allRejectedList.map((element: any, index: number) => {
                  const currentDate = new Date();
                  const formattedDate = currentDate.toLocaleDateString('en-GB', {
                    day: '2-digit',
                    month: '2-digit',
